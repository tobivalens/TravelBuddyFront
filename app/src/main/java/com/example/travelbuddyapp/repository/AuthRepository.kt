...
import com.example.travelbuddyapp.datasource.EditEventData
...
import com.example.travelbuddyapp.datasource.SingleEventData
...
class AuthRepository(
    ...
    suspend fun getAllEvents(): List<EventResponse>?{
        ...
        val token = getAccessToken()
        val response = authService.getAllEvents("Bearer $token")
    ...
    suspend fun getEventById(id: Int): EventResponse?{
        val token = getAccessToken()
        val response = authService.getEventById("Bearer $token", id)
